...
# CẬP NHẬT HEADER: THÊM TEXT CỦA TIN
...
VN_COLS_TRUCSO = {"STT": "STT", "NỘI DUNG": "Nội dung", "ĐỊNH DẠNG": "Định dạng", "NỀN TẢNG": "Nền tảng", "STATUS": "Trạng thái", "NGUỒN": "Nguồn", "NHÂN SỰ": "Nhân sự", "Ý KIẾN ĐIỀU CHỈNH": "Ý kiến", "TEXT CỦA TIN": "Caption/Text", "LINK DUYỆT": "Link Duyệt", "GIỜ ĐĂNG": "Giờ đăng", "NGÀY ĐĂNG": "Ngày đăng", "LINK SẢN PHẨM": "Link SP"}
...
def dinh_dang_dep(wks):
    ...
    set_column_width(wks, 'I', 120); set_column_width(wks, 'J', 200); set_column_width(wks, 'K', 100); set_column_width(wks, 'L', 70); set_column_width(wks, 'M', 80); set_column_width(wks, 'N', 100)
...
if not st.session_state['dang_nhap']:
    ...
    with st.sidebar:
        ...
        selected_menu = option_menu(
            None,
            ["Trực Số", "Checklist Cá Nhân", "Quản Lý Công Việc", "Quản Lý Dự Án", "Lịch Làm Việc", "Email", "Dashboard", "Nhật Ký"] if role == 'LanhDao' else ["Trực Số", "Checklist Cá Nhân", "Quản Lý Công Việc", "Quản Lý Dự Án", "Lịch Làm Việc", "Email"],
            icons=["pencil-square", "check-square", "list-task", "folder", "calendar-week", "envelope", "graph-up", "clock-history"],
            default_index=0,
            styles={
    ...
    if selected_menu == "Trực Số":
        ...
        if exists:
            with st.expander("ℹ️ EKIP TRỰC", expanded=True):
                try:
                    rn = wks_t.row_values(3)[1:]; rr = wks_t.row_values(2)[1:]
                    c = st.columns(4)
                    for i in range(4):
            ...
            with st.form("add_n"):
                ...
                ts_texttin = st.text_area("TEXT CỦA TIN (CAPTION/NỘI DUNG CHỮ)", height=100) # THÊM FIELD MỚI
                ...
                if st.form_submit_button("THÊM VÀO VỎ", type="primary"):
                    with st.spinner("Lưu..."):
                        ...
                        for p in (nt if nt else [""]):
                            # CẬP NHẬT LIST ROW VỚI CỘT MỚI (Vị trí 10: Index 9)
                            wks_t.append_row([start, nd, dd, p, stt, "", ng, ", ".join(ns), yk, ts_texttin, ld, gd.strftime("%H:%M") if gd else "", nda.strftime("%d/%m/%Y"), lsp])
                            start += 1
                ...
                with st.expander("🛠️ SỬA DÒNG TIN", expanded=False):
                    ...
                    if s:
                        ...
                        with st.form("ed_n"):
                            ...
                            e_ns = st.text_input("NS", r['NHÂN SỰ'])
                            # Thêm field sửa Text
                            e_texttin = st.text_area("TEXT CỦA TIN", value=r.get('TEXT CỦA TIN', ''))
                            e_ld = st.text_input("LINK DUYỆT", r['LINK DUYỆT']); e_lsp = st.text_input("LINK SP", r['LINK SẢN PHẨM'])
                            if st.form_submit_button("CẬP NHẬT"):
                                row = idx + 5
                                wks_t.update_cell(row, 2, e_nd)
                                wks_t.update_cell(row, 5, e_st)
                                wks_t.update_cell(row, 8, e_ns)
                                wks_t.update_cell(row, 10, e_texttin) # Cột 10 mới
                                wks_t.update_cell(row, 11, e_ld)      # Cột 11 (Cũ là 10)
                                wks_t.update_cell(row, 14, e_lsp)     # Cột 14 (Cũ là 13)
                                st.success("Xong!"); st.rerun()
                st.dataframe(df_c.drop(columns=['TEXT CỦA TIN'], errors='ignore'), use_container_width=True, hide_index=True) # Ẩn cột Text dài để bảng gọn
